# WaveQuest

Decode the challenge in this intense two-round electronics quiz! Survive the screening round, then battle in the high-pressure Buzzer Finale. From waveform analysis to real-world signal scenarios, prove your signal processing expertise with speed and precision. No devices. No help. Just pure knowledge and quick reflexes. Can you read the signals and claim victory?

## Event Schedule

| Date       | Time     | Reporting Venue    | Duration |
| ---------- | -------- | ------------------ | -------- |
| 07-01-2026 | 09:00 AM | TBD | 3 Hours |



## Rules and Regulations

- Participation: Individual only
- No use of mobile phones, calculators, or electronic devices is allowed
- Format: Round 1 (Screening Quiz) after which qualifiers advance to Round 2 (Buzzer Finale)
- Question Types: Theory, waveform analysis, spectrum interpretation, simulations, real-life signal scenarios
- Strict adherence to time limits and instructions is expected
- Any misconduct, malpractice, or external assistance will lead to disqualification
- Judges' decisions are final

## Organized By

Department: Electronics and Communication Engineering
## For any queries contact:

- Sanjay- 7904750747
- Sai Shivram-9940225293











